...
namespace Brasserie.Utilities.DataAccess
{
    public class DataAccessJsonFile : DataAccess, IDataAccess
    {
        ...
        /// </summary>
        /// <param name="uc"></param>
        public override bool UpdateAllItems(ItemsCollection ic)
        {
            AccessPath = DataFilesManager.DataFiles.GetFilePathByCodeFunction("ITEMS");
            if (IsValidAccessPath)
            {
                JsonSerializerSettings settings = new JsonSerializerSettings { TypeNameHandling = TypeNameHandling.All };
                string json = JsonConvert.SerializeObject(ic, Newtonsoft.Json.Formatting.Indented, settings);
                File.WriteAllText(AccessPath, json);
                return true;
            }
            else
            {
                Console.WriteLine("UpdateAllITems error can't update datasource file");
                return false;
            }
        }
